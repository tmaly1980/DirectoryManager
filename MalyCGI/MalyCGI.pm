...
sub new
{
  ...
  $self->{GLOBAL}->{BASEDIR} = dirname(dirname(__FILE__));
  # Assumes MalyCGI is IN app tree (not above).
...
# MAY HAVE A NEED TO MUCK WITH SOMETHING DB RELATED OR SESSION RELATED...
  ...
  # Moved above ->init() etc since we MIGHT need to modify form and pathinfo

  $self->get_global_vars(); # url's ,etc...
print STDERR "START CGI=$0, $self->{GLOBAL}->{COMPLETE_URL}\n";
  ...
  {
    require MalyDBO;
    my @params = $self->dbinit();
    $self->{GLOBAL}->{DB_PARAMS} = \@params;
    MalyDBO->init(@params);
  ...
  if ($self->{GLOBAL}->{RESTRICTED} and not $self->{GLOBAL}->{SESS_AUTH}) {
  ...
  unless ($self->{GLOBAL}->{NO_SESSION})
  {
    my %params = $self->other_session_process_params;
    $self->{GLOBAL}->{SESSION}->session_process($a,$u,$p,%params);
  }

  if (not $self->{GLOBAL}->{MANUAL_PROCESS})
  {
...
{
  my ($self) = @_;
  return $self->{GLOBAL}->{JUST_LOGGED_IN};

}

sub other_session_process_params { return (); }
sub init {;}
sub subclass_init {;}
...
sub get_vars 
{
  ...
  return
  {
    ...
    PATH_INFO_ARRAY=>$self->{PATH_INFO_ARRAY},
    FORM=>\%form,
    ENV=>\%ENV,
    $self->multirow_vars,
    $self->multipage_vars,
...
sub redirect
{
  ...
  push @headers, (-cookie=>[$self->{GLOBAL}->{SESSION}->get_cookie_headers]);
...
print STDERR "PAGE REDIR TO=$page, CHBP=$self->{GLOBAL}->{COMPLETE_HTML_BASE_PATH}\n";
...
sub process_path_info
{
  ...
  # Will put in array, and possibly in hash, if given!

  my @path_info = split("/", $ENV{PATH_INFO}); 
  shift @path_info if ($ENV{PATH_INFO} =~ m{^/}); # Drop first /

  $self->{PATH_INFO_ARRAY} = [@path_info];
  $self->{PATH_INFO_ARRAY_ORIG} = [@path_info];

  if (ref($keys) eq 'ARRAY')
  {
    my @keys = @{$keys};
    $self->{PATH_INFO} = { map { ($keys[$_], $path_info[$_]) } (0..$#keys) };
    $self->{PATH_INFO_ORIG} = { map { ($keys[$_], $path_info[$_]) } (0..$#keys) };
  }
  # We have ORIG versions so we can see if there genuinely was an explicit 
  # value set or whether or not it was done via a default set
}

sub get_path_info_list
{
  my ($self) = @_;
  return ref $self->{PATH_INFO_ARRAY} eq 'ARRAY' ? @{ $self->{PATH_INFO_ARRAY} } : ();
}
...
sub unset # Erases out of form... even if case doesnt match...
{
  my ($self, @keys) = @_;
  foreach my $rkey (keys %{ $self->{REQUEST} })
  {
    if (grep { uc($rkey) eq uc($_) } @keys)
    {
      delete $self->{REQUEST}->{$rkey};
    }
  }
}
...
{
  my ($self, $key) = @_;
  my $file = $self->{REQUEST}->{_UPLOAD}->{$key}->{data};
  return split(/[\r\n\f]/, $file) if wantarray;
  return $file;
}
...
{
  my ($self, $key) = @_;
  return $self->{REQUEST}->{_UPLOAD}->{$key}->{info};
}
...
sub hash { my ($self, @args) = @_; $self->get_hash(@args); }
sub form { my ($self, @args) = @_; $self->get_hash(@args); }
...
# Everything being XML and needed to be processed in a certain order, we're not going to get ANY content until it's all ready....
# UNLESS (prolly redesign the whole templating!) BLAH...

sub maybe_template_display
{
  my ($self, @vars) = @_;
  $self->{OPTIONAL_TEMPLATE} = 1;
  $self->template_display(@vars);
  $self->{OPTIONAL_TEMPLATE} = 0;
}

sub template_display # Called when we're really ready.
# If not found, will just return. Otherwise, will exit if found template.
{
  my ($self, $page, @tmpl_args) = @_;
  $self->{VARS} = $self->get_vars();
  my $content = $self->{GLOBAL}->{TEMPLATE}->render($page, %{$self->{VARS}}, @tmpl_args);

  return undef if not $content and $self->{OPTIONAL_TEMPLATE};
  # Not important enough to exit() over if missing

  if ($content)
  {
    if (not $self->{GLOBAL}->{SKIP_HEADERS})
    {
      ...
      {
        map { print "Set-Cookie: $_\n"; } $self->{GLOBAL}->{SESSION}->get_cookie_headers();
      }
      print "\n";
...
{
  my ($self, $msg) = @_;
  print STDERR "FROM=".join("; ", caller(1))."\n";
  my $a = $self->get("action");
  my $u = $self->get($self->{GLOBAL}->{SESS_USER});
  my $p = $self->get($self->{GLOBAL}->{SESS_PASS});
  if (ref $self->{GLOBAL}->{SESSION})
  {
    $self->{GLOBAL}->{SESSION}->session_process($a, $u, $p, REQUIRED=>1, MSG=>$msg);
    print STDERR "DONE SESS_PROCESS, REQUIRED=1\n";
  } else {
...
sub login_page
{
  ...
  if (ref $self->{GLOBAL}->{SESSION})
  {
    $self->{GLOBAL}->{SESSION}->session_process("RequiredLogin", $u, $p, REQUIRED=>1, MSG=>$msg);
  } else {
    $self->internal_error("Unable to login, no session processing implemented.");
  }
}
...
{
  my ($self) = @_;
  return ref $self->{GLOBAL}->{DB_PARAMS} eq 'ARRAY' ? @{$self->{GLOBAL}->{DB_PARAMS}} : ();
}
...
{
  my ($self) = @_;
  $self->{GLOBAL}->{SESSION}->get_session_id();
}
