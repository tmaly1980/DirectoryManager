...
sub new
{
  ...
  $self->{INTERNAL_ERROR} ||= $self->{GLOBAL}->{INTERNAL_ERROR} || sub { $self->internal_error(@_) };
  $self->{SYSTEM_ERROR} ||= $self->{GLOBAL}->{SYSTEM_ERROR} || sub { $self->system_error(@_) };
  $self->{USER_ERROR} ||= $self->{GLOBAL}->{USER_ERROR} || sub { $self->user_error(@_) };
  $self->{MSG_PAGE} ||= $self->{GLOBAL}->{MSG_PAGE} || sub { $self->msg_page(@_) };
...
sub enable_log_file
{
  ...
  open(STDERR, ">>$base/tmp/error_log"); # Redirect error log, apache has problems with buffering. BASTARD.
}
...
sub internal_error
{
  ...
  $self->internal_error_text($msg, $page) if $self->{GLOBAL}->{CONTENT_TYPE} eq 'text/plain' or $NOT_CGI;
  $self->internal_error_html($msg, $page);
}
...
sub internal_error_html
{
  ...
  my $base_path = "<base href='$self->{GLOBAL}->{COMPLETE_HTML_BASE_PATH}/'>";
...
<head>
  ...
  $base_path
  <script>
    function goBack() // Might be history, but only if history exists!
    {
      if (history.length > 1)
      {
        history.go(-1);
...
sub system_error
{
  ...
  $self->system_error_text($msg, $page) if $self->{GLOBAL}->{CONTENT_TYPE} eq 'text/plain' or $NOT_CGI;
  $self->system_error_html($msg, $page);
}

sub system_error_text
{
  my ($self, $msg, $page) = @_;
...
sub user_error
{
  ...
  $self->user_error_text($msg, $page) if $self->{GLOBAL}->{CONTENT_TYPE} eq 'text/plain' or $NOT_CGI;
  $self->user_error_html($msg, $page);
}
...
sub msg_page
{
  ...
  $self->msg_page_text($msg, $page) if $self->{GLOBAL}->{CONTENT_TYPE} eq 'text/plain';
  $self->msg_page_html($msg, $page);
}
