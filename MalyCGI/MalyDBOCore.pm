...
sub should_insert # Whether should add/insert (true) or replace/update (false)
{
  ...
  print STDERR "IS_NEW=".$self->is_new."\n";
...
{
  my ($self, @args) = @_;
  my %hash = $self->hash_generic("CURRENT", @args);
  map { (uc $_, $hash{$_}) } keys %hash;
}

sub uc_smart_hash
{
  my ($self, @args) = @_;
  my %hash = $self->uc_hash(@args);
  my %smart_hash = map { ($_, ((ref $hash{$_} eq 'ARRAY' && scalar @{$hash{$_}} <= 1) ? $hash{$_}->[0] : $hash{$_}) ) } keys %hash;
}
...
sub set_unchecked
{
  ...
  foreach my $key (keys %changes)
  {
    ...
    if (!defined $self->{RECORDS}->[$self->{ITER}]->{CURRENT}->{$key} or
      $self->{RECORDS}->[$self->{ITER}]->{CURRENT}->{$key} ne $value)
    {
      # If undef, or if current value is different than what setting to, set.
      # We don't want to put unnecessary changes into 'CHANGES'.
      $self->{RECORDS}->[$self->{ITER}]->{CHANGES}->{$key} = $value;
      $self->{RECORDS}->[$self->{ITER}]->{CURRENT}->{$key} = $value;

      # So we don't check pseudo values when we explicitly set.
      $self->{RECORDS}->[$self->{ITER}]->{CHANGES}->{_EXPLICIT_SET}->{$key} = 1;
      $self->{RECORDS}->[$self->{ITER}]->{CURRENT}->{_EXPLICIT_SET}->{$key} = 1;
    }
  }
...
sub commit # Does save to database whether insert or update.
{
  ...
  $self->set(@params) if @params; # So if we get the key this way, we know to not add
  ...
  {
    #$self->insert(@params); # Don't pass, already set.
    $self->insert();
  } elsif ($self->has_pending_changes) {
    #$self->update(@params); # Don't pass, already set.
    $self->update(); 
  }
  return $self if defined wantarray;
}
...
sub subclass_init { $LOGGER->internal_error("Schema unititialized. Override subclass_init."); } 
...
sub get_pseudo_meta # To disable, we must EXPLICITLY set depth(0)
{
  ...
  #print STDERR "PSEUDO=".Dumper($self->{PSEUDO})."\n" if $self->{BOGUS};
