...
sub struct_update
{
  ...
  $self->struct_update_core(undef, $struct, $pseudo, %hash);
}

sub struct_update_undef_delete
{
  my ($self, $struct, $pseudo, %hash) = @_;
  $self->struct_update_core(1, $struct, $pseudo, %hash);
}

sub struct_update_core # Updates arbitrary structure with a flat hash, which is converted and merged.
{
  # Stuff may be invalid like BLAH{FLUKE}_POOP....
  # so traverse to point where parent should be, and just abort, not erase!
  my ($self, $undef_delete, $struct, $pseudo, %hash) = @_;
  foreach my $key (keys %hash)
  {
    ...
    if ($undef_delete and not defined $hash{$key})
    {
      my $new_parent_struct = $$parent_struct;
      if (ref $$parent_struct eq 'ARRAY')
      {
        splice @{ $$parent_struct }, $name, 1;
      } elsif (ref $$parent_struct eq 'HASH') {
        delete $$parent_struct->{$name};
      } else { # Else, don't know what to do!
        #This is okay, it just wasnt there in the first place!
	
	# BUT FOR SOME REASON, it gets saved... FIX!!! XXX TODO

        #my $ref = ref $$parent_struct;
	#my $val = $$parent_struct;
        #print STDERR "TRYING TO ALTER UNKNOWN PARENT STRUCTURE! WE'RE AT A $ref, WAS=$val, TRYING TO SET '$key' TO UNDEF\n";
      }
    } else {
...
sub var_evaluate # Array, hash or scalar.
{
  ...
  my ($varname, $remain) = $var =~ /^([\w:+_-]+)(.*)/; # var name is everything up until first { or [
  my $func = undef;
  ...
  if ($remain)
  {
    ...
    my @keys = ref $vars eq 'Search' ? %$vars : ();
    my $keys = join(";", @keys);
...
sub func_evaluate
{
  ...
  $func =~ /^digits([_](\d+))?/ && return ((length(int($v))||'0')+$2);
  $func eq 'signed' && return ($v > 0 ? "+$v" : $v); 
  $func eq 'lc' && return (lc($v));
  $func eq 'money' && return (money($v));
  $func eq 'moneycents' && return (money($v, 1));
  $func eq 'formalnum' && return (formalnum($v));
  $func =~ /^formalnum_(\d+)$/ && return (formalnum($v, $1));
  $func eq 'uc' && return (uc($v));
  ...
  $func eq 'keys' && (return get_keys($v));
...
sub get_keys # Works on arrays too!
# If wanting scalar, will return # instead.
{
  my ($v) = @_;
  my $i = 0;
  if (ref $v eq 'ARRAY')
...
sub formalnum # With commas in proper place. And given decimal points, or none if not specified.
{
  my ($num, $dec) = @_;
  my $num = $dec > 0 ? sprintf "%.${dec}f" : int($num);
  while ($num=~s/([0-9])([0-9][0-9][0-9])\b/$1,$2/) {}
  return $num;
}
...
{
  my ($amt, $cents) = @_;
  my $num = $cents ? formalnum($amt, 2) : formalnum($amt);
  return "\$$num";
}
...
sub make_list
{
  ...
  @values = map { ($_, $append) } @values if $append;
  my $list = join($char, @values);
}
...
sub get_pseudo_value # Should return undef when binding criteria matches (up-to-date), or no _PSEUDO_key
{
  ...
  #print STDERR "PSEUDO_VAR=$psuedo_var\n" if $key eq 'VIEW_URL';

  return undef unless $pseudo_var;
  return undef if $vars->{_EXPLICIT_SET}->{$key}; # Don't bother if we explicitly set from ->set();
  ...
  if (ref $pseudo_var eq 'ARRAY') # Otherwise, it's a simple MalyVar mapping?
  {
	...
	# FIX BOTH WAYS, CHANGING @records to USE OF RECS->more()!!!
      ...
      # If $srckey is an array, we need to get a LIST
      my $dstvalue = undef;
      if (ref $srckey eq 'ARRAY')
      {
        my @values = ();
	foreach my $sk (@$srckey)
	{
	  my $vals = $self->var_evaluate($vars, $sk, $pseudo, 1);
	  push @values, ref $vals eq 'ARRAY' ? @$vals : $vals;
	}
	$dstvalue = \@values;
      } else {
        $dstvalue = $self->var_evaluate($vars, $srckey, $pseudo, 1);
      }
  ...
  $dbo->{DEBUG} = ($key eq 'SUBTASK_IDS');
    ...
    #print STDERR "EVALUATED $pseudo_var AS=$vars->{$key}, VAR=$vars->{UID}, ALLVARS=".Dumper($vars).", ALL PSEUDO=".Dumper($pseudo)."\n" if $key eq 'VIEW_URL';
  }
...
sub get_hash_value
{
  ...
  if (ref $vars and isa($vars, "MalyDBOCore")) # Object.
  {
    ...
    if ($key) { # Want to get sub record whose key has certain value.
      ...
      # Always check pseudo_value, skip, if already properly set and up-to-date
      $self->get_pseudo_value($vars, $value, $pseudo);
    ...
    # RE_ENABLED NEXT TWO LINES, TO ALLOW GETTING THE HASHREF OF A DB RECORD WHERE THERE IS ONLY ONE. 01/23/2004
      ...
      $self->get_pseudo_value($vars->[0], $value, $pseudo);
...
sub evaluate_content # Separate to make easy to remember
{
  ...
  #print STDERR "FROM=".join("; ", caller(6))."\n";
  #print STDERR "FROM=".join("; ", caller(8))."\n";
  #print STDERR "FROM=".join("\n", map { join("; ", caller($_)) } (0..5))."\n";
  ...
  while( $content =~ s/(?<!\\)#([\w:+_-]+?(\[[\w#=+_-]*?\]|\{[\w#=+_-]*?\})*)#/$self->flat_var_evaluate($vars, $1, $pseudo, $all)/ge ) {;}
...
sub get 
{
  my ($class, $struct, $key, $pseudo) = @_;

  if (not $key) # JUST dereferencing $struct, i s'pose
  {
    if (wantarray)
    {
      return %$struct if ref $struct eq 'HASH';
  ...
  my $rv = MalyVar->var_evaluate($struct, $key, $pseudo);
  if (wantarray)
  {
