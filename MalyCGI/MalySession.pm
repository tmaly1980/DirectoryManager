...
sub new
{
  ...
  $self->{COOKIE} = {};
  die("No CGI Object passed!") unless ref $self->{GLOBAL}->{CGI};
  ...
  MalyLog->enable_log_file();

  if (not $self->{GLOBAL}->{NO_SESSION})
  {
...
{
  my ($self, $a, $u, $p, %params) = @_;
  my $restricted = $params{RESTRICTED} || $params{REQUIRED};
  my $msg = $params{MSG};
  ...
  print STDERR "SESSION=".Dumper($self->{SESSION_INFO})."\n" if $self->{GLOBAL}->{SESSION_DEBUG};


  #$self->{GLOBAL}->{LOGGER}->log("COOKIE=$cookie,SESSINFO=".Dumper($self->{SESSION_INFO}));

  if ($a eq 'RequiredLogin') # Don't even accept cookie!
  {
    $self->{GLOBAL}->{SESS_LOGIN_PAGE}->($msg, undef);

  } elsif ( ($a eq 'Login' or $self->{GLOBAL}->{RESTRICTED} or $restricted) and not ($u or $p) and not $cookie)
  ...
  elsif ($a eq 'Logout') # Remove session
  {
    ...
    $self->{GLOBAL}->{SESS_LOGOUT_PAGE}->(undef, $cookie);
  }
  elsif ($a eq "Login" and $u and $p) # Actually authenticate session now.
  {
    print STDERR "CALLING VERIFY LOGIN (based upon (LOGIN action)\n" if $self->{GLOBAL}->{SESSION_DEBUG};
    return $self->verify_login($self->{GLOBAL}->{SESS_USER}=>$u,PASSWD=>$p,RESTRICTED=>1,ACTION=>$a,%params);
  }
  elsif (not $cookie)
  {
    print STDERR "CALLING VERIFY LOGIN\n" if $self->{GLOBAL}->{SESSION_DEBUG};
    return $self->verify_login($self->{GLOBAL}->{SESS_USER}=>$u,PASSWD=>$p,RESTRICTED=>$self->{GLOBAL}->{RESTRICTED}||$restricted, ACTION=>$a,%params);
  }
  return 0;
}
...
# If PURPOSELY want to go to self, should pass URL to self. Else, would prefer to go to POST_LOGIN_PAGE
  ...
  $self->process_login($username, $passwd, %params);
  ...
  return 0;
    ...
    #{
    #  $next_page = "$self->{GLOBAL}->{HTML_BASE_PATH}/$self->{GLOBAL}->{POST_LOGIN_PAGE}";
    #}
    #elsif ($params{POST_LOGIN_PAGE})
    #{
    #  $next_page = "$self->{GLOBAL}->{HTML_BASE_PATH}/$params->{POST_LOGIN_PAGE}";
...
sub get_cookie_headers # Suitable for use with HTTP headers, with 'path', 'exp', etc.
{
  my ($self) = @_;
  my %cookies = ref $self->{COOKIE} eq 'HASH' ? %{ $self->{COOKIE} } : ();
  my @cookies = ();
  foreach my $name (keys %cookies)
  {
    my %params = ref $self->{COOKIE_PARAMS}->{$name} eq 'HASH' ? 
      %{ $self->{COOKIE_PARAMS}->{$name} } : ();
    my $cookie = $self->{GLOBAL}->{CGI}->cookie(-name=>$name,-value=>$cookies{$name},%params);
    push @cookies, $cookie;
  }
  return @cookies;
}

# No support quite yet for paths and expirations, etc.... always assume at end of session
sub cookie # Store in a hash.
{
  my ($self, $name, $value, @params) = @_;
print STDERR "CALLING SESSION->COOKIE($name,$value) FROM=".join(", ", caller())."\n";
  my $cookie = undef;
  my %params = @params;
  if (@_ > 2) # Set as well
  {
    print STDERR "SETTING COOKIE $name TO $value\n"; #if $self->{GLOBAL}->{SESSION_DEBUG};
    $params{-expires} ||= $self->{GLOBAL}->{SESS_LENGTH}||undef;
    $params{-path} ||= $self->{GLOBAL}->{HTML_BASE_PATH}||'/';
    $cookie = $self->{COOKIE}->{$name} = $value;
    $self->{COOKIE_PARAMS}->{$name} = \%params if %params;
  } else {
    $cookie = $self->{COOKIE}->{$name};
  }
  if (defined wantarray)
  {
    print STDERR "RETURNING $cookie\n";
    return $cookie;
  }
}

sub process_cookies # Just to set and send back, no session associated with.
{
  my ($self) = @_;
  my @cookienames = $self->{GLOBAL}->{CGI}->cookie();
  my @auxcookienames = ref $self->{GLOBAL}->{AUX_COOKIES} eq 'ARRAY' ? 
    @{ $self->{GLOBAL}->{AUX_COOKIES} } : ();

  foreach my $cookie (@cookienames, @auxcookienames)
  {
    my $cgi_value = $self->{GLOBAL}->{CGI}->param($cookie);
    my $value = $self->{GLOBAL}->{CGI}->cookie(-name=>$cookie) || $cgi_value;
print STDERR "COOKIE=$cookie,VALUE=$value\n";
    if ($value)
    {
...
sub get_session_id
{
  ...
  my $session_id = $self->cookie($self->{GLOBAL}->{SESSION_ID_NAME});
print STDERR "SESSION_ID=$session_id\n";
  if (not $session_id and $cgi_session_id)
  {
    ...
    $self->cookie($cookie_name,$session_id,
      -path=>$self->{GLOBAL}->{HTML_BASE_PATH} || "/",
...
{
  my ($self, $username, $passwd, %params) = @_;
  ...
  if ($username and $passwd) # Not logged in, or trying to log in again, check.
  {
    ...
    print STDERR "SESS_ATUH=$username, $password\n";
    my ($new_username, %extra) = $self->{GLOBAL}->{SESS_AUTH}->($username, $passwd, %params);
    ...
    $self->session_create($username, $passwd, $session_id, %extra);
  }
...
sub session_create
{
  my ($self, $username, $passwd, $session_id, %extra) = @_;
...
sub default_delete_session
{
  ...
  unlink($session_path);
  #or $self->{GLOBAL}->{LOGGER}->internal_error("Unable to remove session (at $session_path): $!");
  # It's OK to not remove it properly! In case it's not there!
...
# Call this to get the cookie needed to delete the session (and to delete it internally).
{
  ...
  my $cookie = $self->cookie($self->{GLOBAL}->{SESSION_ID_NAME},$session_id,
    -path=>$self->{GLOBAL}->{HTML_BASE_PATH},-expires=>"-1y");
...
# SHOULDWORK WITH MIXED CASE !!!
  ...
  if (isa($self->{SESSION_INFO}, "MalyDBOCore")) # Object. Must implement get()
  {
    ...
    } elsif (@vars and defined wantarray) { # Asking for scalar. (NOT going to call records()!)
      $values[0] = $self->{SESSION_INFO}->get(@vars);
    } elsif (not @vars and wantarray) {
      return $self->{SESSION_INFO}->hash;
    } elsif (not @vars and defined wantarray) { 
      return $self->{SESSION_INFO}->hashref;
    }
  } else {
    if (@vars == 0)
    {
      if (wantarray)
      {
        return ref $self->{SESSION_INFO} eq 'HASH' ? %{ $self->{SESSION_INFO} } : ();
      } elsif (defined wantarray) {
        return $self->{SESSION_INFO};
      }
    }
    elsif (@vars == 1 and wantarray) # ALso try here to get implicit records()
    {
...
sub default_logout_page
{
  my ($self, $cookie) = @_;
  my $header = $self->{GLOBAL}->{CGI}->header(-cookie=>$cookie);
  $self->{GLOBAL}->{SESS_LOGIN_PAGE}->(undef, $cookie) if ($self->{GLOBAL}->{LOGOUT_TO_LOGIN});
  if ($self->{GLOBAL}->{STYLE})
  {
    my $style_path = $self->{GLOBAL}->{STYLE};
    $style_path = "$self->{GLOBAL}->{COMPLETE_HTML_BASE_PATH}/$style_path" unless $style_path =~ m{^/};
