...
function set_checkbox(checkbox, val) // Checkbox (one or more)
{
  ...
  if (checkbox.length)
  {
    ...
    if (default_value)
    {
      var default_values = multiple ? default_value.split(",") : [default_value];
...
{
    var default_value = arguments[2];
    radiogroup = get_field(radiogroup);
    ...
    if (radiogroup.length)
    {
      ...
      {
        found = set_radio(radiogroup[i], val);
      }
        ...
        for (var i = 0; i < radiogroup.length; i++)
	{
      ...
      if (radiogroup.value == val)
      {
        radiogroup.checked = 1;
	return 1;
      }
      if (radiogroup.value == default_value)
      {
        radiogroup.checked = 1;
	return 1;
      }
...
function set_multiselect(sel, val) // Select box, with multiple values. 'val' is probably an array.
{
    ...
    if (typeof val == 'string') { val = val.split(delim); } 
    var found = 0;
    ...
    // Not found, set to one with default value.
    if (typeof default_value == 'string') { default_value = default_value.split(delim); } 
    if (! default_value.length) { default_value = [default_value]; }
    for (var i = 0; i < sel.options.length; i++)
    {
      var opt = sel.options[i];

      for (var j  = 0; j < default_value.length; j++)
      {
...
function prompt_get() // joins the values from one or more prompts, with ':' delimiting. Return empty string if no values in anything.
{
  // RIGHT NOW ONLY IMPLEMENTS ONE PROMPT!!!! TODO
  var msg = arguments[0] || "Enter in a value: ";
  var values = new Array;
  var default_values = arguments[1];
  var delim  = arguments[2] || ':';

  // convert if not array
  if (msg && (typeof msg == 'string' || !msg.length)) { msg = [msg]; } // Force into array.

  // convert if not array
  if (default_values && (typeof default_values == 'string' || !default_values.length)) { default_values = [default_values]; } // Force into array.

  for (var i = 0; i < msg.length; i++)
  {
    var value = window.prompt(msg[i],(default_values && default_values.length ? default_values[i] : ''));
    values.push(value);
  }
...
function text_get(text) // joins the values from one or more text fields, with ':' delimiting. Return empty string if no values in anything.
{
  ...
  text = get_field(text);
  var text_list;
  if (text && (text.type == 'text' || text.type == 'select-multiple'))
  {
    text_list = [text];
...
function dualMultiList_save(mainlist, sublist, sublist_text)
{
  ...
  clear_select(sublist);
  if (sublist_text) { sublist_text.value = ""; }
...
function multiList_prompt_replace(list)
// Replace existing value.
{
  ...
  var delim = arguments[3] || ':';
  multiList_prompt_modify(list, msg, list.selectedIndex, format, delim);
  return false;
}


function multiList_prompt_add(list)
{
  var msg = arguments[1];
  var format = arguments[2];
  var delim = arguments[3] || ':';
  multiList_prompt_modify(list, msg, -1, format, delim);
  return false;
}

// SPECIFY DISPLAY FORMAT VIA SPRINTF!!! PASSED TO MULTILIST ADD/REPLACE/EDIT!

function multiList_prompt_modify(list, msg, replace_index)
{
  ...
  // Need to get prvious value, i.e. as done by _edit

  var previous_value;

  if (replace_index == -1) 
  { 
    replace_index = list.options.length; 
  } else {
    previous_value = list.options[replace_index].value;
  }

  var text_value = prompt_get(msg, previous_value, delim);
...
// Copy from list to text.
{
  ...
  if (text && text.length && ! text.type) // Array
  {
    text_set(text, value, delim);
  } else if (text) { // Just text.
    text.value = value;
  }
  return false;
}
...
function set_field(f, v) // Tries to figure out what is, calls appropriate function
{
  ...
  // This is broken as to not set radios properly when multiple !!!
  // 
  var d = arguments[2];
  f = get_field(f);
  ...
  if (type == 'select-one')
  {
    return set_select(f, v, d);
  } else if (type == 'select-multiple') {
    return set_multiselect(f, v, d);
  } else if (type == 'text' || type == 'hidden' || type == 'password') {
    return set_text(f, v, d);
  } else if (type == 'textarea') {
    return set_text(f, v, d);
  } else if (type == 'checkbox') {
    return set_checkbox(f, v, d);
  } else if (type == 'radio') {
    return set_radio(f, v, d);
  } else if (f.length) { // Node list
    for (var i = 0; i < f.length; i++)
    {
      found = set_field(f[i], v);
      // need to fix for textboxes...
      if(found) { break; }
    }
...
function set_text(f, v, d)
{
  ...
  if (f.type != 'text' && f.type != 'textarea' && f.type != 'hidden' && f.type != 'password') { return false; }
  if (!v && d)
  {
    f.value = d;
    return d;
  ...
  function isEnabledSection(sect)
  {
    sect = get_field(sect);
    return (sect && sect.style && sect.style.display == 'block') ? 1 : 0;
  }

function check_all(master, prefix)
{
  master = get_field(master);
  var box;
  for (var i = 0; box = get_field(prefix+'_'+i); i++)
  {
    if (master.checked)
    {
      box.checked = 1;
...
function toggleField(src, fields) // Value of '1' means to disable.
{
    var src_field = document.getElementById(src);
    var value;
    if (src_field)
    {
      value = get_field_value(src_field);
    } else {
      value = src;
    }
    ...
    {
      fields[i] = get_field(fields[i]);
      if (fields[i])
      {
        fields[i].disabled = (value > 0 ? true : false);
      }
    }
}
